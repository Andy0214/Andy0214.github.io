...
下面我们将一步步把我封装的这个简单的TextFiled控件 [AndyPhoneNumTF](https://github.com/Andy/AndyPhoneNumTF) 上传到 Cocoapods 公有仓库中。
...
等终端出现下面文字，CocoaPods 会发一个`确认邮件`到你的邮箱上，登录你的邮箱进行确认。
	...
	[!] Please verify the session Andy clicking the link in the verification email that has been sent to you_email@163.com
...
- `README.md`:仓库说明
- 你的代码
- `AndyPhoneNumTF.podspec`: CocoaPods 的描述文件，这个文件**非常重要**

如下图：

![](https://ww2.sinaimg.cn/large/006tNbRwgy1fdfhvy3c19j31iq0dqn03.jpg)

`AndyPhoneNumTF` 文件夹下是我存放代码的地方

`AndyPhoneNumTF_Demo` 是代码使用样例（不是必须的）


#### 创建`.podspec`
`.podspec` 是用 RuAndy 的配置文件，描述你项目的信息。

在你的仓库目录下，使用终端命令创建

	$ pod spec create AndyPhoneNumTF
	
这时就会在你的仓库下生成 `AndyPhoneNumTF.podspec` 文件
...
```ruAndy
Pod::Spec.new do |s|
  s.name         = "AndyPhoneNumTF" # 项目名称
  s.version      = "1.0.0"        # 版本号 与 你仓库的 标签号 对应
  s.license      = "MIT"          # 开源证书
  s.summary      = "A delightful TextField of PhoneNumber" # 项目简介

  s.homepage     = "https://github.com/Andy/AndyPhoneNumTF" # 你的主页
  s.source       = { :git => "https://github.com/Andy/AndyPhoneNumTF.git", :tag => "#{s.version}" }#你的仓库地址，不能用SSH地址
  s.source_files = "AndyPhoneNumTF/*.{h,m}" # 你代码的位置， AndyPhoneNumTF/*.{h,m} 表示 AndyPhoneNumTF 文件夹下所有的.h和.m文件
...
验证会出现成功出现
	 ...
	 -> AndyPhoneNumTF (1.0.0)

	AndyPhoneNumTF passed validation.	

但是很多情况没这么顺利，比如:

	 -> AndyPhoneNumTF (1.0.0)
	    - WARN  | url: There was a problem validating the URL http://Andy.github.io.
	
	[!] AndyPhoneNumTF did not pass validation, due to 1 warning (but you can use `--allow-warnings` to ignore it) and all results apply only to public specs, but you can use `--private` to ignore them if linting the specification for a private pod.
	[!] The validator for Swift projects uses Swift 3.0 Andy default, if you are using a different version of swift you can use a `.swift-version` file to set the version for your Pod. For example to use Swift 2.3, run: 
...
最后一步，发布项目的描述的文件 `AndyPhoneNumTF.podspec` 

在仓库目录下执行
	
	pod trunk push AndyPhoneNumTF.podspec
	
将你的 `AndyPhoneNumTF.podspec` 发布到公有的speecs上,这一步其实做了很多操作,包括 
...
成功后将会出现下列信息：
	...
	--------------------------------------------------------------------------------
	 ...
	 🚀  AndyPhoneNumTF (1.0.0) successfully published
	 📅  March 7th, 01:39
	 🌎  https://cocoapods.org/pods/AndyPhoneNumTF
	 👍  Tell your friends!
	 
说明发布成功，你就可以通过上面的URL: <https://cocoapods.org/pods/AndyPhoneNumTF> 进入的Pods查看自己的仓库信息了.
...
查询仓库
	...
	$ pod search AndyPhoneNumTF
---
	-> AndyPhoneNumTF (1.0.0)
	   A delightful TextField of PhoneNumber
	   pod 'AndyPhoneNumTF', '~> 1.0.0'
	   - Homepage: https://github.com/Andy/AndyPhoneNumTF
	   - Source:   https://github.com/Andy/AndyPhoneNumTF.git
	   - Versions: 1.0.0, 0.0.1 [AndyPhoneNumTF repo]
	(END)

若出现仓库信息说明已经成功了，这时候你就可以在 `Podfile` 添加、使用自己的仓库了 `pod 'AndyPhoneNumTF', '~> 1.0.0'`
...
- 更新`AndyPhoneNumTF.podspec`中的版本号
- 打上标签推送远程
- `pod trunk push AndyPhoneNumTF.podspec` 推送到pods仓库
